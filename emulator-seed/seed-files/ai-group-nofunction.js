...
async function seedAIGroup(auth, db) {
  ...
  // Write all messages to Firestore WITH FUNCTIONS DISABLED
  console.log('  ✓ Writing 50 messages to Firestore (functions disabled)...');

  // Get test environment to disable functions
  ...
  await testEnv.withSecurityRulesDisabled(async (context) => {
    ...
    messages.forEach(msg => {
      ...
      // Convert Firebase Admin Timestamps to plain JS objects
      const convertedData = {
        ...messageData,
        timestamp: messageData.timestamp instanceof Timestamp
          ? { seconds: messageData.timestamp.seconds, nanoseconds: messageData.timestamp.nanoseconds }
          : messageData.timestamp,
        readBy: Object.keys(messageData.readBy || {}).reduce((acc, key) => {
          const ts = messageData.readBy[key];
          acc[key] = ts instanceof Timestamp
      ...
      batch.set(testDb.collection('messages').doc(id), convertedData);
    });
